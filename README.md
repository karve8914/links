...
- 自動清除網址末尾常見標點
- 同一 PDF、同一網址會合併為一列，並列出所有出現頁碼
- 同一網址若出現在不同檔案，會分開列出，避免檔案與頁碼對錯
- 檢查結果只顯示：`可連線` / `無法連線`
- 可匯出 CSV
- 可一鍵複製所有「無法連線」網址
...
- 結果表保留「檔案」欄，新增「PDF 頁碼」欄。
- 同一網址若在同一 PDF 出現在多頁，例如第 3、7、12 頁，會顯示為 `3、7、12`。
- 同一網址若存在於不同 PDF，會依檔案分成不同列。
- DOCX 不嘗試推算頁碼，頁碼欄顯示 `—`。Word 的實際頁碼會受到紙張、字型、段落、印表機與版面設定影響，瀏覽器解析 DOCX 無法可靠重現。


## 重複網址列示方式（v1.3）

- 同一網址在文件中出現多次時，**每一次出現都保留為獨立一列**，不再合併成單一列。
- 但每一列的「PDF 頁碼」都會顯示該網址在同一份 PDF 中出現過的**全部頁數**。
- 例如 `https://example.com` 分別出現在第 3、7 頁，結果會有兩列；兩列的 PDF 頁碼都顯示 `3、7`。
- 若同一網址在第 3 頁出現 2 次、第 7 頁出現 1 次，會保留 3 列；每列頁碼仍顯示 `3、7`。
- PDF 解析時會用兩種文字排列策略交叉辨識，但同一頁同一網址的出現次數採兩種策略中的較大值，避免解析方式造成假的重複紀錄。


## 目前的重複網址規則（v1.4）

- 同一檔案中的相同網址只保留 **1 列**。
- 「PDF 頁碼」會列出該網址在同一份 PDF 出現過的所有頁數。
- 例如同一網址出現在第 3、7 頁，結果只顯示一列，頁碼欄為 `3、7`。
- 即使同一網址在第 3 頁出現 2 次、第 7 頁出現 1 次，仍只顯示一列，頁碼欄仍為 `3、7`。
- 相同網址若出現在不同檔案，會依檔案分開列出。
