...
app.innerHTML = `
  <main class="wrap">
    ...
    <section class="card">
      ...
      <div class="note">Word 支援 .docx；舊式 .doc 請先在 Word 另存為 .docx。掃描型 PDF 若整頁都是圖片，需先做 OCR，否則無法從頁面文字擷取網址。</div>
    </section>
...
function cleanUrl(raw) {
  let u = String(raw || '').trim().replace(/^<|>$/g, '').replace(TRAILING_RE, '');
...
function extractUrlsFromText(text, source, location = '') {
  ...
  for (const match of String(text || '').matchAll(URL_RE)) {
    ...
    if (url) out.push({ url, source, location });
  }
  return out;
}
...
async function extractPdf(file) {
  ...
  for (let pageNo = 1; pageNo <= pdf.numPages; pageNo += 1) {
    ...
    found.push(...extractUrlsFromText(text, file.name, `第 ${pageNo} 頁`));
