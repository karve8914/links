  ...
  <main class="wrap">
    ...
    <section class="card">
      ...
      <div class="table-wrap">
        <table>
          <thead>
            <tr><th style="width:60px">序號</th><th style="width:170px">檔案</th><th>網址</th><th style="width:120px">結果</th></tr>
          </thead>
          <tbody id="tbody"><tr><td colspan="4" class="empty">尚無檢查結果</td></tr></tbody>
        </table>
      </div>
      <div class="note">文件內容會在你的瀏覽器本機解析；後端只會收到擷取後的網址，不會收到 PDF 或 Word 檔案本身。</div>
    </section>
  </main>
`;
...
function collectCandidates(text) {
  ...
  const candidates = [];
  // 若成功修復斷行，優先只掃描修復後文字，避免同時留下半截網址。
  const view = repaired;

  for (const re of [PROTOCOL_RE, WWW_RE, BARE_DOMAIN_RE]) {
    re.lastIndex = 0;
    for (const match of view.matchAll(re)) candidates.push(match[0]);
  }
  return candidates;
}
...
async function extractPdf(file) {
  ...
  for (let pageNo = 1; pageNo <= pdf.numPages; pageNo += 1) {
    ...
    const lineAwareText = content.items.map((item) => `${item.str || ''}${item.hasEOL ? '\n' : ''}`).join('');
    found.push(...extractUrlsFromText(spacedText, file.name, `第 ${pageNo} 頁`));
    found.push(...extractUrlsFromText(lineAwareText, file.name, `第 ${pageNo} 頁`));

    const annotations = await page.getAnnotations();
    for (const a of annotations) {
      const candidate = a.url || a.unsafeUrl || '';
      const url = cleanUrl(candidate);
      if (url) found.push({ url, source: file.name, location: `第 ${pageNo} 頁超連結` });
    }
  }
  return found;
}
...
function uniqueRecords(items) {
  ...
  for (const item of items) {
    ...
    if (!map.has(key)) {
      ...
      if (!row.sources.includes(item.source)) row.sources.push(item.source);
    }
  }
  return [...map.values()];
}
...
function renderResults() {
  ...
  if (!results.length) {
    tbody.innerHTML = '<tr><td colspan="4" class="empty">尚無檢查結果</td></tr>';
...
checkBtn.addEventListener('click', async () => {
  ...
  try {
    ...
    const rows = uniqueRecords(extracted);
    if (!rows.length) {
    ...
    statusText.textContent = `完成，共 ${rows.length} 個不重複網址`;
  } catch (error) {
    console.error(error);
...
exportBtn.addEventListener('click', () => {
  const rows = [['序號', '來源檔案', '網址', '結果']];
  results.forEach((r, i) => rows.push([i + 1, (r.sources || [r.source]).join('、'), r.url, r.ok ? '可連線' : '無法連線']));
