...
- 可辨識網址直接夾在中文句子中，不要求網址前後有空白。
- 可正確處理全形標點，例如：`產業節能減碳資訊網（2024）：https://ghg.tgpf.org.tw，經濟部產業發展署。`
- 可辨識 `http://`、`https://`、`www.`、`www2.` 等形式。
- 可辨識常見裸網域，例如 `moe.gov.tw`。
- 自動移除零寬空白、soft hyphen 等肉眼不易發現的字元。
- 對網址在 `/`、`.`、`?`、`&`、`=` 等結構符號後換行的情況，會嘗試安全重組。
- PDF 與 DOCX 仍會另外讀取文件本身設定的超連結。

網址文字主體以常見 ASCII URL 字元為主要辨識範圍，這是為了避免 `https://example.com查詢` 把後面的中文字誤判成網址。若文件中的可見網址含「未編碼的中文字路徑」，建議以文件本身的可點擊超連結為準。
